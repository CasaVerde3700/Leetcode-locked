...
defined and is ambiguous for k > 2 cases. If "Zigzag" does not look right to you, replace "Zigzag" with 
...
public class ZigzagIterator implements Iterator<Integer> {
	...
	public ZigzagIterator(List<Iterator<Integer>> list) {
		this.iters = new LinkedList<Iterator<Integer>>();
		// add valid iterators
	...
	public Integer next() {
		...
		int idx = turns % iters.size(); // important
		Iterator<Integer> cur = iters.get(idx);
		Integer res = cur.next();
